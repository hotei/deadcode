...
func notused() {
	fmt.Printf("Never used\n")
}

func init() {
	flag.BoolVar(&makeFlag, "make", true, "Use with make - no error if deadcode found")
	flag.BoolVar(&mdFlag, "md", false, "Format output to be friendly to markdown")
	flag.BoolVar(&verboseFlag, "verbose", false, "be chatty")
}

// errorf adds program identification and a newline
func errorf(format string, args ...interface{}) {
	errString := fmt.Sprintf("deadcode: "+format+"\n", args...)
	if makeFlag {
...
func main() {
	flag.Parse()
	if mdFlag { // tell markdown parser to leave formatting alone
		fmt.Printf("```\ndeadcode results:\n\n")
		defer func() {
			fmt.Printf("```\n")
		}()
